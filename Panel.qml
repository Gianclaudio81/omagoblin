...
Panel {
  ...
  function refreshActiveModel() {
    if (!activeModelProcess.running) activeModelProcess.running = true
  }
  ...
  Process {
    id: activeModelProcess
    running: false
    command: ["bash", Qt.resolvedUrl("active-model.sh").toString().replace("file://", "")]

    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var model = String(text || "").trim()
        if (/^[A-Za-z0-9._-]+$/.test(model)) root.activeModelId = model
      }
    }
  }
  ...
  Timer {
    ...
    onTriggered: root.refreshActiveModel()
  }
